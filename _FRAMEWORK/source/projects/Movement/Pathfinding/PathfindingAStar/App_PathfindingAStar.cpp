...
#pragma region UI
	//UI
	{
		...
		if (ImGui::Combo("Pathfinding Type", &m_SelectedPathFinding, "AStar\0ThetaStar\0ThetaStarPS\0AnyAnglePS\0JPS\0JPSPS\0RandomWalk\0", 6))
		{
...
void App_PathfindingAStar::CalculatePath()
{
		...
		&& m_startPathId != m_endPathId)
	{
		...
		Elite::GraphNode* pStartNode = m_pTerrainGraph->GetNode(m_startPathId);
		Elite::GraphNode* pEndNode = m_pTerrainGraph->GetNode(m_endPathId);
		
		//Select the intended pathfinding
		switch (m_SelectedPathFinding)
		{
			case int(PathFindingType::AStar) :
			{
				std::unique_ptr<AStar> pPathFinder = std::make_unique<AStar>(m_pTerrainGraph, m_heuristicFunction);
				m_vPath = pPathFinder.get()->FindPath(pStartNode, pEndNode);
			}
			break;

			case int(PathFindingType::ThetaStar) :
			{
				std::unique_ptr<ThetaStar> pPathFinder = std::make_unique<ThetaStar>(m_pTerrainGraph, m_heuristicFunction);
				m_vPath = pPathFinder.get()->FindPath(pStartNode, pEndNode);
			}
			break;

			case int(PathFindingType::ThetaStarPS) :
			{
				std::unique_ptr<ThetaStarPS> pPathFinder = std::make_unique<ThetaStarPS>(m_pTerrainGraph, m_heuristicFunction);
				m_vPath = pPathFinder.get()->FindPath(pStartNode, pEndNode);
			}
			break;

			case int(PathFindingType::AnyAnglePS) :
			{
				std::unique_ptr<ThetaStarAnyAnglePS> pPathFinder = std::make_unique<ThetaStarAnyAnglePS>(m_pTerrainGraph, m_heuristicFunction);
				m_vPath = pPathFinder.get()->FindPath(pStartNode, pEndNode);
			}
			break;

			case int(PathFindingType::JPS) :
			{
				std::unique_ptr<JPS> pPathFinder = std::make_unique<JPS>(m_pTerrainGraph, m_heuristicFunction);
				m_vPath = pPathFinder.get()->FindPath(pStartNode, pEndNode);
			}
			break;

			case int(PathFindingType::JPSPS) :
			{
				std::unique_ptr<JPSPS> pPathFinder = std::make_unique<JPSPS>(m_pTerrainGraph, m_heuristicFunction);
				m_vPath = pPathFinder.get()->FindPath(pStartNode, pEndNode);
			}
			break;

			case int(PathFindingType::RandomWalk) :
			{
				std::unique_ptr<RandomWalk> pPathFinder = std::make_unique<RandomWalk>(m_pTerrainGraph, m_heuristicFunction);
				m_vPath = pPathFinder.get()->FindPath(pStartNode, pEndNode);
			}
			break;
...
void App_PathfindingAStar::UpdateAgentPath(const std::vector<Elite::GraphNode*>& path)
{
	...
	for (const Elite::GraphNode* pNode : path)
	{
